...
function renderTimeline(data) {
    ...
    results += '<h6>Timeline for ' + escapeHtml(record.first_name) + ' ' + escapeHtml(record.last_name) +
    ...
    api.campaignId.results(campaign.id)
        .success(function(c) {
            ...
            if (campaign) {
                ...
                $chart.on('click', '.ct-timeline-point', function() {
                    var $point = $(this)
                    cidx = $point.attr('meta')
                    html = "Event: " + campaign.timeline[cidx].message
                    if (campaign.timeline[cidx].email) {
                        var email = campaign.timeline[cidx].email;
                        var time = campaign.timeline[cidx].time;
                        var anchorName = encodeURI(email);
                        ...
                        if (rowNum !== -1) {
                            var rowsPerPage = resultsTable.page.len();
                            var numRows = rows.length;
                            var pageNum = Math.floor(rowNum/rowsPerPage);
                            resultsTable.page(pageNum).draw(false);
